...
class Bot {
  ...
  def respond(input: String) = {
    ...
    if (opCode == "React") {
      ...
      view.offsetToNearest(Cells.GoodBeast) match {
...
object Actions {
  ...
  def paramsFrom(paramMap: Map[String,String]) = paramMap.map(param => param._1 + "=" + param._2).mkString(",")

  def actionFrom(opCode: String, paramMap: Map[String,String]) = opCode + "(" + paramsFrom(paramMap) + ")"

  def Log(text: String) = actionFrom("Move", Map("text" -> text))

  def Move(direction: XY) = Move(direction.toString)

  def Move(direction: String) = actionFrom("Move", Map("direction" -> direction))

  def Spawn(direction: XY, name: String, energy: Int) = Spawn(direction.toString, name, energy)

  def Spawn(direction: String, name: String, energy: Int) = {
    if (energy < 100) {
      Log("Not enough energy to spawn!")
    } else {
      actionFrom("Spawn", Map(
      "direction" -> direction,
  ...
  def Set(paramMap: Map[String,String]) = {

    val illegalNames = Array("generation", "name", "energy", "time", "view", "direction", "master", "collision")

    def illegalCharsIn(s: String) = ",=()|".exists(s.contains)

    val paramErrors: Array[Option[String]] = paramMap.map(param => {
      val (key, value) = (param._1, param._2)
      if (illegalCharsIn(key)) {
        Some("key: \"" + key + "\"")
      } else if (illegalCharsIn(value)) {
        Some("value: \"" + key + "\"")
      } else {
        None
      }
    ...
    //TODO: Map over paramErrors, if some, error happens, else continue
